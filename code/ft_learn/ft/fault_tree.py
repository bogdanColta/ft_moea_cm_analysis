...
class FaultTree:
    ...
    def phi_prec(self, dataset):
        ...
        return 1 - (self.TP ) / (self.TP + self.FP) if self.TP + self.FP else 1
    
    def phi_spec(self, dataset):
        if self.N == None:
            self.compute_confusion_matrix_attributes(dataset)
        
        return 1 - (self.TN ) / (self.TN + self.FP) if self.TN + self.FP else 1
    ...
    def phi_sens(self, dataset):
        ...
        return 1 - (self.TP ) / (self.TP + self.FN) if self.TP + self.FN else 1
    
    def phi_npv(self, dataset):
        if self.N == None:
            self.compute_confusion_matrix_attributes(dataset)
        
        return 1 - (self.TN ) / (self.TN + self.FN) if self.TN + self.FN else 1
    
    def phi_fnr(self, dataset):
        if self.N == None:
            self.compute_confusion_matrix_attributes(dataset)
        
        return (self.FN) / (self.P) if self.P else 1
    
    def phi_fpr(self, dataset):
        if self.N == None:
            self.compute_confusion_matrix_attributes(dataset)
        
        return (self.FP) / (self.N) if self.N else 1
    
    def phi_fdr(self, dataset):
        if self.N == None:
            self.compute_confusion_matrix_attributes(dataset)
        
        return (self.FP) / (self.FP + self.TP) if self.TP + self.FP else 1
    
    def phi_for(self, dataset):
        if self.N == None:
            self.compute_confusion_matrix_attributes(dataset)
        
        return (self.FN) / (self.FN + self.TN) if self.FN + self.TN else 1
    
    def phi_acc(self, dataset):
        if self.N == None:
            self.compute_confusion_matrix_attributes(dataset)
        
        return 1 - (self.TP + self.TN) / (self.P + self.N) if self.P + self.N else 1

    def phi_ts(self, dataset):
        if self.N == None:
            self.compute_confusion_matrix_attributes(dataset)
        
        return 1 - (self.TP) / (self.TP + self.FN + self.FP) if self.TP + self.FP + self.FN else 1
    
    def phi_bacc(self, dataset):
        if self.N == None:
            self.compute_confusion_matrix_attributes(dataset)
        
        return 1 - (self.TP/self.P + self.TN/self.N) / 2 if self.P and self.N else 1
    
    def phi_F1(self, dataset):
        if self.N == None:
            self.compute_confusion_matrix_attributes(dataset)
        
        return 1 - (2*self.TP/(2*self.TP + self.FN + self.FP)) if 2*self.TP + self.FP + self.FN else 1
    ...
    def phi_nlr(self, dataset):
        ...
        return fnr / tnr if tnr != 0 else 1
    ...
    def phi_npr(self, dataset):
        ...
        lr_plus = sensitivity / denominator if denominator != 0 else 1
    ...
    def phi_dor(self, dataset):
        ...
        dor = lr_plus / lr_minus if lr_minus != 0 else 1
        minimized_dor_value = 1 / dor if dor != 0 else 1
    ...
    def phi_kappa(self, dataset):
        ...
        return 1 - (po - pe) / (1 - pe) if 1 - pe else 1
